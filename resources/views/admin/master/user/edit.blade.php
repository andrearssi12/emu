...
@section('content')
    ...
    <div
        ...
        <div class="flex justify-between mb-5">
            <h1 class="text-gray-900 dark:text-white font-bold">Tabel Data User</h1>
        ...
        <form action="{{ route('user.update', $user->hashed_id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-12 mt-5 md:items-center">
                ...
                <div class="flex col-span-12 md:col-span-6">
                    ...
                    <input type="email" id="email" name="email"
                        ...
                        placeholder="@gmail.com" value="{{ old('email', $user->email ?? '') }}">
                </div>
                @error('email')
                    <div class="col-span-12 md:col-span-6 mt-1">
                        <p class="text-sm text-red-600">{{ $message }}</p>
            ...
            <div class="grid grid-cols-12 mt-5 md:items-center">
                <label for="status"
                    class="col-span-12 md:col-span-2 xl:col-span-1 block mb-2 text-sm font-medium text-gray-900 dark:text-white">Status</label>
                <div class="col-span-12 md:col-span-6">
                    <!-- Hidden input for default value when checkbox is not checked -->
                    <input type="hidden" name="status" value="Nonaktif">

                    <div class="flex items-center">
                        <input id="status" name="status" type="checkbox" value="Aktif"
                            {{ old('status', $user->status) == 'Aktif' ? 'checked' : '' }}
                            class="focus:ring-blue-500 h-4 w-4 text-blue-600 border-gray-300 rounded dark:bg-gray-700 dark:border-gray-600 dark:focus:ring-blue-500 dark:ring-offset-gray-800">
                        <label for="status" class="ml-2 text-sm font-medium text-gray-900 dark:text-white">
                            Aktif
                        </label>
                    </div>
                    <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">
                        {{ old('status', $user->status) == 'Aktif' ? 'Akun ini aktif.' : 'Jika checkbox tercentang, maka akun ini aktif.' }}
                    </p>
                    @error('status')
                        <div class="mt-1">
                            <p class="text-sm text-red-600">{{ $message }}</p>
